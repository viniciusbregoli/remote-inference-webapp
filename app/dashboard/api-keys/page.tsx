...
type ApiKeyWithUser = APIKey & { user: { username: string } };
...
export default function ApiKeysPage() {
  ...
  const fetchUsers = async () => {
      try {
          ...
          // Non-critical, modal just won't have users to select
          console.error(err);
      }
  }
        ...
        {apiKeys.length === 0 && !isLoading ? (
          <div className="text-center py-12 px-6">
            ...
            <p className="mt-1 text-sm text-gray-500">
              No API keys exist in the system yet.
            </p>
            {isAdmin && (
              <div className="mt-6">
                <Button
                ...
                >
                  <Plus size={18} className="mr-1" /> Create First Key
                </Button>
              </div>
            )}
          </div>
                ...
                {apiKeys.map((key) => (
                  <tr key={key.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {key.user.username || `ID: ${key.user_id}`}
                    </td>
